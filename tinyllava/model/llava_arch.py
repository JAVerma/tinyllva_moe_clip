...
from tinyllava.model.multimodal_encoder.builder import build_vision_tower,build_vision_towers
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.vision_tower = build_vision_towers(config, delay_load=True)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower() is None:
        ...
        elif self.get_vision_tower()[0].vision_tower_name != vision_tower:
            print(f"rebuilding vision tower! vision tower initialized from: {vision_tower}")
            # vision_tower = build_vision_tower(model_args)
            vision_tower = build_vision_towers(model_args)
            if fsdp is not None and len(fsdp) > 0:
                self.vision_tower = [vision_tower]
            else:
                self.vision_tower = vision_tower
            ...
            for vision_t in vision_tower:
                vision_t.load_model()
        self.config.use_mm_proj = True
        self.config.mm_projector_type = getattr(model_args, 'mm_projector_type', 'linear')
        # self.config.mm_hidden_size = vision_tower.hidden_size
        self.config.mm_hidden_size = vision_tower[0].hidden_size+ vision_tower[1].hidden_size 
